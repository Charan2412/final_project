...
def search_names(query):
    print(query)
    similar_rows = list(df[df['product_name'].str.contains(query, case=False) | df['brands'].str.contains(query, case=False)]['product_name'])
    print(similar_rows)
    return similar_rows
...
def profile():
    ...
    if request.method == 'POST':
        ...
        cursor.execute('UPDATE users SET username = %s, password = %s, age = %s, gender = %s, health_conditions = %s, allergies = %s, height = %s, weight = %s WHERE id = %s',
